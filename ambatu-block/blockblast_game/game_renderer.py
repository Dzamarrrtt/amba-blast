...
class BlockGameRenderer:
    ...
    def __init__(self, game_state, fps=60):
        ...
        pygame.display.set_caption("Amba Blast Game")
        ...
        backsound_path = os.path.join(assets_dir, "backsound.mp3")
        if not os.path.isfile(backsound_path):
            print(f"Warning: Sound file not found at {backsound_path}, using silent sound.")
            self.clear_backsound = pygame.mixer.Sound(buffer=bytearray(10))
        else:
            pygame.mixer.init()
            pygame.mixer.music.load(backsound_path)   # Ganti dengan nama file kamu
            pygame.mixer.music.set_volume(0.5)         # Volume dari 0.0 (diam) ke 1.0 (maks)
